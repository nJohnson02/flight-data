...
def detect_approaches(records, order=150, min_climb=500, lookback_seconds=180, field_elevation=5045, elevation_tolerance=500, altitude_buffer=500):
    ...
    for idx in minima_indices:
        ...
        for j in range(max(0, idx - order * 3), idx):
            ...
            if altitudes[j] >= a_min + min_climb:
                # Find where altitude first dropped below a_min + altitude_buffer
                start_idx = j
                for k in range(j, idx):
                    if altitudes[k] <= a_min + altitude_buffer:
                        start_idx = k
                        break
                approach = {
...
# Example usage
log = "log_231023_184242_KPRC"
csv_path = "/media/nathan/bulk-storage/prescott-flight-logs/" + log + ".csv"
